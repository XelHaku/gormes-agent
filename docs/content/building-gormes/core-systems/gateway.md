...
- **Telegram fresh-final streaming** — Hermes `b16f9d43` sends a fresh final Telegram message for long-lived streamed previews so visible timestamps reflect completion time. Gormes tracks this as a shared gateway coalescer policy row followed by Telegram config + deleteMessage wiring under Phase 2.B.5.
