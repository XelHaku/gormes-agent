...
| 5 / 5.O | CLI tips/dump/webhook deterministic helpers (deprecated umbrella) | `tools` | The row is selected at all — execute the three sibling rows above (CLI deterministic tip selector, CLI webhook URL normalizer, CLI dump support-summary helper) instead. | - |
