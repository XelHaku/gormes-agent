...
| 2 / 2.F.5 | Steer slash command registry + queue fallback | Registry-owned /steer command accepts operator/gateway text during a busy turn, validates/truncates preview text, and queues visible guidance when no running-agent steer hook is available | operator, gateway | `internal/gateway/steer_command_test.go` | Unblocks Mid-run steer injection between tool calls, Gateway-handled slash commands bypass active-session guard. |
