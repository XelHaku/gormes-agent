...
| 4 / 4.A | Bedrock stale-client eviction + retry classification | Bedrock SigV4 + credential seam | Bedrock SigV4 + credential seam is complete, so a fake runtime client/cache seam can be exercised without live AWS credentials., This slice only needs a tiny in-memory region->client cache and a stale-error classifier; it must not implement request signing, stream decoding, model discovery, or provider registration. | - |
