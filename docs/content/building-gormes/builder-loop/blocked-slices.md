...
| 4 / 4.A | Cross-provider reasoning-tag sanitization | DeepSeek/Kimi all-assistant reasoning_content replay, Codex stream repair + tool-call leak sanitizer | DeepSeek/Kimi reasoning_content replay and Codex stream repair are complete on main, so stored assistant reasoning/text boundaries already exist., The worker can implement and test a pure sanitizer in internal/hermes with synthetic strings; no live provider, kernel wiring, TUI rendering, or transcript migration is required. | Gemini, OpenRouter, Google Code Assist, Codex OAuth state + stale-token relogin |
