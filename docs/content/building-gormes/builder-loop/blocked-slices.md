...
| 2 / 2.B.5 | Telegram fresh-final delete and config exposure | Gateway fresh-final send/delete fallback | Gateway fresh-final send/delete fallback is complete and exposes ManagerConfig.FreshFinalAfter plus the optional MessageDeleter interface., Gormes uses TOML config, so the setting should live on config.TelegramCfg as fresh_final_after_seconds rather than copying Hermes YAML structures., go-telegram-bot-api/v5 exposes tgbotapi.NewDeleteMessage and BotAPI.Request for deleteMessage; tests should assert the Chattable shape through the mock client, not call Telegram. | - |
