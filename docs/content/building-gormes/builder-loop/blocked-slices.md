...
| 4 / 4.A | Bedrock SigV4 + credential seam | Bedrock stream event decoding (SSE fixtures) | Bedrock stream event decoding (SSE fixtures) is complete so request mapping and response decoding are fixture-backed before signing is introduced., The slice can use a fixed http.Request, fixed time `2026-04-26T22:45:56Z`, and literal fake credentials; no AWS SDK, boto3, IMDS, profile-file read, or network call is required., Secrets must appear only in local test literals and never in evidence strings, errors, or status output. | Bedrock stale-client eviction + retry classification |
