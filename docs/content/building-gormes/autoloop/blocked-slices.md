...
| 4 / 4.B | Aux compression provider-aware context cap | Provider-enforced context-length resolver, Aux compression headroom for system and tool schemas | Provider-enforced context-length resolver is validated on main., Aux compression headroom for system and tool schemas is validated on main., The slice can use fake ModelContextResolver inputs for openai-codex gpt-5.5 raw 1050000 tokens vs provider cap 272000 tokens without live provider calls. | Tool-result pruning + protected head/tail summary, Manual compression feedback + context references |
...
| 5 / 5.A | Terminal process watch notification throttle contract | Tool registry inventory + schema parity harness | Tool registry inventory + schema parity harness is validated on main., The first slice owns a pure fake-clock notification policy under internal/tools and does not need to spawn a real shell or wire the terminal tool handler. | Stateful tool migration queue, Dangerous-command detector + blocked-result schema, PTY bridge protocol adapter |
| 5 / 5.I | Dashboard page-scoped plugin slot inventory | Plugin SDK, Dashboard theme/plugin extension status contract | Plugin SDK and dashboard extension status contracts are validated on main., The slice can use synthetic dashboard/manifest.json fixtures and the native /api/dashboard/plugins handler without embedding upstream React/Vite assets. | - |
...
| 5 / 5.O | Update service restart active polling | Gateway, platform, webhook, and cron management CLI | Gateway, platform, webhook, and cron management CLI has established a service-manager seam, or this slice owns a pure helper with a fake service runner and no command wiring., Installer source-backed update flow remains documented as Gormes' current public update path. | - |
