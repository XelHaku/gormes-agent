...
| 4 / 4.B | Aux compression headroom for system and tool schemas | Compression token-budget trigger + summary sizing, Provider-enforced context-length resolver | Compression token-budget trigger + summary sizing is validated on main., Provider-enforced context-length resolver is validated on main., The test can estimate tool-schema overhead from local descriptors or synthetic ToolDescriptor fixtures without opening a provider connection. | Tool-result pruning + protected head/tail summary, Manual compression feedback + context references |
...
| 4 / 4.H | Generic unsupported-parameter retry + max_tokens guard | Unsupported temperature retry + Codex flush guard | Unsupported temperature retry + Codex flush guard is validated on main., Gormes HTTP providers serialize MaxTokens and Temperature through fake HTTP tests without live credentials. | OpenRouter, Codex OAuth state + stale-token relogin, Provider rate guard + budget telemetry |
...
| 5 / 5.O | Effective toolset picker dedupes bundled plugin keys | Plugin SDK, First-party Spotify plugin fixture, Platform toolset config persistence + MCP sentinel | Plugin SDK is validated on main., First-party Spotify plugin fixture is validated on main., Platform toolset config persistence + MCP sentinel is validated on main. | - |
