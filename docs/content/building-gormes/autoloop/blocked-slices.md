...
| 5 / 5.O | Oneshot noninteractive safety and clarify policy | Oneshot final-output writer boundary | Oneshot final-output writer boundary is validated on main., Dangerous-command and clarify fixtures can use fake tools/providers without interactive stdin or live credentials. | - |
| 5 / 5.O | Service restart active-status poller | Service RestartSec parser helper | Service RestartSec parser helper is validated on main., The poller can run with a fake runner, fake clock, and no command wiring; later Gateway/platform/webhook/cron CLI rows can consume it. | - |
