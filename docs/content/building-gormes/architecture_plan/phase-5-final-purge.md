...
| 5.O — Hermes CLI Parity | ⏳ planned | Port the 49-file `hermes_cli/` tree as dependency-aware command groups: deterministic helpers, PTY bridge adapter, registry/active-turn/busy policy, config/profile/auth/setup, top-level `-z/--oneshot` with model/provider resolution, platform toolset persistence/MCP sentinel behavior, RestartSec-aware service restart polling, gateway/platform/webhook/cron management, diagnostics/backup/logs/status; replaces the upstream `hermes` binary |
...
Hermes `5006b220` adds two CLI/service deltas that Gormes should freeze as small rows before broad CLI ports: top-level one-shot mode (`hermes -z`) with explicit model/provider/env resolution, and update restart polling that waits through systemd `RestartSec` before declaring the gateway failed to relaunch. Neither row should pull in the full interactive CLI tree.
...
8. **5.O one-shot mode splits parser from execution.** First land top-level `-z/--oneshot`, `--model`, `--provider`, and env fallback parsing with no TUI/api_server startup. Then land stdout-only native-kernel execution over fake clients. This prevents workers from mixing public CLI semantics, provider routing, output capture, and safety policy in one large attempt.
9. **5.O service restart polling is pure helper work first.** Parse fake `RestartUSec` output and compute `max(10s, RestartSec+10s)` before any command wiring touches real `systemctl`, Windows service control, installers, or gateway restart logic.
