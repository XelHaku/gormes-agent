...
8. **5.O one-shot mode stays split after parser validation.** Top-level `-z/--oneshot`, `--model`, `--provider`, and env fallback parsing are validated on main. Next land the final-output writer boundary over fake clients, then the noninteractive safety/clarify policy. This prevents workers from mixing stdout capture with approval semantics.
