...
| **Phase 2.E.2 — OS-AI Spine: Concurrent-Tool Cancellation** | ✅ complete | **P1** | Kernel-side interrupt propagation is landed: `internal/kernel.executeToolCallsConcurrent` fans out tool_calls with bounded concurrency, propagates one parent-context cancel to every in-flight and queued worker, and returns a single coherent cancel envelope (`{"error":"cancelled","reason":"parent_cancelled"}`) in preserved input order, freezing the seam that parallel-tool execution and 2.F.5 mid-run steering will wire in |
...
| Phase 2.F.3 — Restart / Pairing / Status | 🔨 in progress | P2 | Graceful shutdown drain and the in-process `StatusModel` channel-lifecycle writer (registered/running/disconnected/failed) are landed in `internal/gateway`; next slices are adapter startup cleanup, active-turn follow-up/late-arrival drain policy, drain-timeout resume recovery, pairing persistence, pairing approval/rate-limit semantics, a read-only status command, runtime-status JSON/PID validation, token-scoped credential locks, then `/restart` takeover markers |
...
5. **P2 — 2.F.3 status readout + runtime convergence**
   Add `gormes gateway status` as a read-only view over configured channels plus pairing/runtime state, port runtime-status JSON/PID validation, then add token-scoped credential locks and `/restart` takeover/dedup markers. The in-process `gateway.StatusModel` write side is already landed (`internal/gateway/status.go`), so these on-disk slices wrap that seam rather than introducing a parallel one.
