...
| Phase 2.F.3 — Restart / Pairing / Status | 🔨 in progress | P2 | Graceful shutdown drain and the `/restart` takeover-marker contract (registry command, dedup on redelivered updates, pluggable service-manager hook) are landed in `internal/gateway`; remaining slices are adapter startup cleanup, active-turn follow-up/late-arrival drain policy, drain-timeout resume recovery, pairing persistence, pairing approval/rate-limit semantics, a read-only status command, runtime-status JSON/PID validation, token-scoped credential locks, persistent RestartMarkerStore + cmd wiring, then channel lifecycle writers |
...
5. **P2 — 2.F.3 status readout + runtime convergence**
   Add `gormes gateway status` as a read-only view over configured channels plus pairing/runtime state, port runtime-status JSON/PID validation, and add token-scoped credential locks before threading live lifecycle updates into that model. The `/restart` takeover-marker contract is already frozen on `gateway.Manager` (registry command + dedup of redelivered updates); the remaining lifecycle-layer work is a persistent `RestartMarkerStore` and `cmd/gormes gateway` wiring for real service-manager exit semantics.
