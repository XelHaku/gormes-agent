...
| Restart / pairing / lifecycle | `gateway/{restart,pairing,status}.py`, `gateway/platforms/base.py` + `PairingStore` | 2.F | 🔨 graceful drain is landed in `internal/gateway/manager.go`; `/restart` takeover/dedup markers are frozen at the manager contract (persistence + cmd wiring pending); remaining Go slices are adapter startup cleanup, active-turn follow-up/late-arrival drain policy, drain-timeout resume recovery, pairing persistence, approval/rate-limit semantics, status JSON/PID validation, token-scoped credential locks, read-only status CLI, and live lifecycle writers |
