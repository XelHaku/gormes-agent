...
| WhatsApp | `gateway/platforms/whatsapp.py` | 2.B.4 | 🔨 in progress | Transport-neutral ingress normalization, command passthrough, and bridge-vs-native runtime selection live in `internal/channels/whatsapp`; pairing/reconnect/send still remains |
...
| BlueBubbles (iMessage) | `gateway/platforms/bluebubbles.py` | 7.E | ✅ shipped | Webhook-auth, cached chat-GUID resolution, and home-channel send fallback live in `internal/channels/bluebubbles`; further runtime work is paused behind the priority channel set |
