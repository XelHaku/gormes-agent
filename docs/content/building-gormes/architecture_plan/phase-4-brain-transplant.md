...
| 4.B — Context Engine + Compression | ⏳ planned | The ContextEngine status boundary is landed. Next port the pure compressor budget slice from Hermes `5401a008` so model switches recalculate threshold, tail, and max-summary budgets before message pruning, summarization, or context references land |
...
12. **4.B compressor budget state + model-switch recalculation** — mirror Hermes `5401a008` in pure Go fixtures: initialization and model updates must recalculate `threshold_tokens`, `tail_token_budget`, and `max_summary_tokens` from the active context window, with no provider credentials and no kernel history mutation.
