...
Telegram is not a greenfield port for Gormes. `internal/channels/telegram/` already ships the shared gateway adapter, so the question here is which PicoClaw edge mechanics are worth pulling in as follow-up deltas.
...
- Current shipped Gormes behavior was verified in-tree against `internal/channels/telegram/bot.go`, `internal/channels/telegram/client.go`, and `gormes/cmd/gormes/gateway.go`.
...
The donor is less useful for inbound runtime structure because `internal/channels/telegram/bot.go` already owns the transport loop and hands normalized events to the shared gateway manager.
...
- Inbound lifecycle. `internal/channels/telegram/bot.go` is built around `gateway.InboundEvent`, `gateway.ParseInboundText`, and `gateway.Channel`; do not import PicoClaw's bus-oriented `HandleInboundContext` call graph.
...
- PicoClaw `Start` long-poll loop maps conceptually to `(*Bot).Run` in `internal/channels/telegram/bot.go`.
...
- Treat Markdown work as a delta on top of `internal/channels/telegram`, not a reason to replace the existing shared-manager coalescer. Gormes already has the streamed-edit behavior that PicoClaw implements through placeholder and message-edit capabilities.
