...
PLANNER_TIMER_NAME="${PLANNER_TIMER_NAME:-gormes-architectureplanneragent}"
...
install_systemd_timer() {
  ...
  script_path="$SCRIPT_DIR/architectureplanneragent.sh"
...
[Unit]
Description=Gormes architecture planner agent
...
EOF
  ...
  cat > "$timer_file" <<EOF
[Unit]
Description=Run the Gormes architecture planner periodically
