...
// Tips is the Go port of hermes_cli.tips.TIPS (Phase 5.O) — a flat corpus
// of CLI one-liners that describe slash commands, @ context references,
// keybindings, CLI flags, config toggles, gateway/skills/cron hints, and
// power-user tricks. Keep this list in sync with upstream on corpus edits;
// internal/cli/tips_test.go pins the expected count plus the first/last
// anchor entries so silent drops fail loudly.
var Tips = []string{
...
// PickRandomTip mirrors hermes_cli.tips.get_random_tip. It selects one
// entry from Tips using the supplied *rand.Rand so tests can inject a
// deterministic source. A nil *rand.Rand falls back to the package-level
// math/rand/v2 generator, matching upstream random.choice behaviour.
//
// excludeRecent is retained for signature parity with upstream; the
// underlying deduplication-across-sessions behaviour is not yet wired.
func PickRandomTip(r *rand.Rand, excludeRecent int) string {
	if len(Tips) == 0 {
		return ""
	}
	if r == nil {
...
// GetRandomTip is the direct parity alias for hermes_cli.tips.get_random_tip.
// It selects from the package-level random source so callers ported from
// Python keep the same call site.
func GetRandomTip(excludeRecent int) string {
	return PickRandomTip(nil, excludeRecent)
}
