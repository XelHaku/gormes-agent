...
func TestAutoCodexuOrchestratorLoopsByDefaultWhenBacklogEmpty(t *testing.T) {
	...
	copyOrchestratorLib(t, repoRoot, tmpRepo)
	writeFile(t,
...
9) Acceptance check
Criterion: selected task was claimed once — PASS
Criterion: progress.json was committed complete — PASS
Criterion: integration promotion should include the worker commit — PASS
EOF
`), 0o755)
...
9) Acceptance check
Criterion: selected task was claimed — PASS
Criterion: non-zero codex exit kept a valid report — PASS
Criterion: integration promotion should include the worker commit — PASS
EOF
...
func copyOrchestratorLib(t *testing.T, repoRoot, tmpRepo string) {
	t.Helper()

	srcRoot := filepath.Join(repoRoot, "scripts", "orchestrator", "lib")
	entries, err := os.ReadDir(srcRoot)
	if err != nil {
		t.Fatalf("read %s: %v", srcRoot, err)
	}
	for _, entry := range entries {
		...
		name := entry.Name()
		copyFile(t,
			filepath.Join(srcRoot, name),
			filepath.Join(tmpRepo, "scripts", "orchestrator", "lib", name),
			0o644,
		)
	}
}
