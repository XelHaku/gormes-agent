...
| MCP integration | `tools/{mcp_tool,mcp_oauth,mcp_oauth_manager,managed_tool_gateway}.py` + `mcp_serve.py` + `MCPOAuthManager`, `MCPServerTask`, `ManagedToolGatewayConfig`, `SamplingHandler`, `OAuthNonInteractiveError`, `_ManagedFalSyncClient` classes | 5.G | 🔨 in progress — `internal/mcp/client.go` now provides the Go-native stdio client core (`initialize` + `notifications/initialized`, negotiated protocol validation, `tools/list`, `tools/call`); HTTP transport, dynamic registry injection, server mode, sampling, and OAuth remain follow-on scope |
