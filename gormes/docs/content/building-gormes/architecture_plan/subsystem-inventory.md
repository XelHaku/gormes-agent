...
| Operator tools | `tools/{todo_tool,clarify_tool,session_search_tool,send_message_tool,debug_helpers,interrupt,ansi_strip}.py` + `TodoStore`, `_ThreadAwareEventProxy` classes | 5.N | 🔨 in progress — `internal/tools/clarify.go` now ports the clarify-tool core with validated question/choice payloads, 120s timeout, structured replies, and dormant default-registry wiring, while `internal/tools/debug_helpers.go` now ports the shared redacting `DebugSession` helper with explicit-save persistence under `${XDG_DATA_HOME}/gormes/logs`; sibling operator tools remain planned |
