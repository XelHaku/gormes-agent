...
| 5.F — Skills System (Remaining) | 🔨 in progress | `internal/skills` now ships the typed skill-registry layer (`SkillMeta`, `SkillBundle`, `SkillReadinessStatus`, `HubLockFile`), deterministic filesystem/static source discovery, the canonical bundled/official/Claude Marketplace/ClawHub/GitHub/Hermes Index/LobeHub/skills.sh registry catalog, and the first local hub/install runtime. `cmd/gormes/skills.go` now syncs materialized `.hub/catalogs/<source>/.../SKILL.md` trees into `.hub/cache` + `lock.json`, lists hub or installed skills, and installs hub refs into the active skills store under `${XDG_DATA_HOME}/gormes/skills/`. Remaining work is skill auto-discovery, security/audit/update flows, and broader category backfill. **Note:** Core learning loop (skill extraction, improvement) is Phase 2.G (P0) |
