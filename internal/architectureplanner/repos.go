...
func SyncExternalRepos(ctx context.Context, cfg Config, runner autoloop.Runner) ([]RepoSyncResult, error) {
	if runner == nil {
...
func syncExternalRepo(ctx context.Context, cfg Config, runner autoloop.Runner, repo ExternalRepo) (RepoSyncResult, error) {
...
func commandOutput(result autoloop.Result) string {
	if result.Stdout != "" {
